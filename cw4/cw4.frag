...
vec3 computeShadow(in Intersection intersect)
{
	...
	// Compute shadow if there's some obstruction in between
	intersect.hit = 0;
...
vec3 computeMultiShadow(in Intersection intersect, vec3 light)
{
//	Ray shadow[numRay];
	Ray shadow;
	...
	vec3 temp;
	  shadow.origin = intersect.point + epsilon * intersect.normal;
	  shadow.dir = normalize(light - shadow.origin);
	  // Compute shadow if there's some obstruction in between
	  intersect.hit = 0;
	  //Check if the shadow(secodary) ray is blocked by other object 
	  Intersect(shadow, intersect);
...
//	    contr = contr / (rd + 1);
		...
		for (int l = 0; l < numRay ; l++){
		contr = contr / (rd + l + 1);
	  	outcolour += contr * vec4(computeMultiShadow(i, light[l]),1);
		}
...
//	  r.dir = reflect(r.dir, i.normal);
	  // assume the balls are glasses and have ior = 1.3
